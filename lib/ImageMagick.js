...
class ImageMagick {
    ...
    im.identify(attachment.path, (error, attributes) => {
      ...
      Object.keys(this._options.transforms).forEach((name) => {
        ...
        const outputFile = path.join(this._options.tmpDir,
          path.parse(attachment.path).name + '_' + name + extension
        )

        this._setUpTransform(attachment, transform, outputFile, tasks, model[name], doc, storageProvider)
      })
  ...
  _setUpTransform (attachment, transform, outputFile, tasks, model, doc, storageProvider) {
    const self = this
